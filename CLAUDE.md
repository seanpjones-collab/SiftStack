...
- **Two-layer cross-run dedup** — `seen_case_numbers` (per-bucket case_no ledger) catches the SAME case across runs (ALN's 6-week republish cycle). `seen_addresses` (global state:zip5:normalized_street ledger) catches the SAME PROPERTY across buckets and across re-filed cases (e.g. a re-filed foreclosure under a fresh case_no). Both apply pre-enrichment so we don't pay Smarty/Tracerfy on dupes; both filter against a run-start snapshot so the current run's own records aren't dropped. **Probate is EXEMPT from `seen_addresses`** — a death is a fresh motivation signal that must ship even if the property previously shipped as a foreclosure/tax notice, and probate addresses are unreliable at dedup time anyway (Cuyahoga/Stark have none yet; Summit carries CourtView's decedent *residence*, often a shared facility → false-positive drops). `seen_case_numbers` still stops the exact same probate case from re-shipping.
